namespace RT_MediaOps.Plan.Workflow.Filtering
{
	...
	public sealed class JobFilteringTests
	{
		...
		private Tuple<Job[], FilterElement<Job>>[] JobFilterTestCases => new[]
		{
			...
			new Tuple<Job[], FilterElement<Job>>([Setup.DraftJob1!, Setup.DraftJob2!], JobFilter.AND(JobExposers.ActionRequired .Equal(true))),
			new Tuple<Job[], FilterElement<Job>>([Setup.TentativeJob3!], JobFilter.AND(JobExposers.ActionRequired.Equal(false))),
