namespace RT_MediaOps.Plan.RST.Resources
{
	...
	public sealed class CapabilityAssignmentTests : IDisposable
	{
		...
		public void Dom_SingleResourceCapability()
		{
			...
			var profileParameterIdField = domResourceCapability.FieldValues.SingleOrDefault(f => f.FieldDescriptorID.Id == Storage.DOM.SlcResource_Studio.SlcResource_StudioIds.Sections.ResourceCapabilities.ProfileParameterID.Id);
			Assert.IsNotNull(profileParameterIdField);
			Assert.IsTrue(Guid.TryParse(Convert.ToString(profileParameterIdField.Value.Value), out var profileParameterId));
			Assert.AreEqual(capability.Id, profileParameterId);

			var stringValueField = domResourceCapability.FieldValues.SingleOrDefault(f => f.FieldDescriptorID.Id == Storage.DOM.SlcResource_Studio.SlcResource_StudioIds.Sections.ResourceCapabilities.StringValue.Id);
			Assert.IsNotNull(stringValueField);
			var discreteValues = Convert.ToString(stringValueField.Value.Value).Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries).ToList();
