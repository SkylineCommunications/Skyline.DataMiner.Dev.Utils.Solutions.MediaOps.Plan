namespace Skyline.DataMiner.Solutions.MediaOps.Plan.API
{
	...
	internal class ResourcePoolsRepository : Repository, IResourcePoolsRepository
	{
		...
		/// <returns>A read-only collection containing the created resource pools.</returns>
		...
		/// <returns>A read-only collection containing the created or updated resource pools.</returns>
		/// <exception cref="ArgumentNullException">Thrown when <paramref name="apiObjects"/> is <c>null</c>.</exception>
		...
		public void Delete(IEnumerable<ResourcePool> apiObjects)
		{
			...
			Delete(apiObjects.Select(x => x.Id).ToArray(), defaultOptions);
		}
		...
		public void Delete(IEnumerable<Guid> apiObjectIds)
		{
			...
			var defaultOptions = new ResourcePoolDeleteOptions();
			Delete(apiObjectIds, defaultOptions);
		}
		...
		public void Delete(ResourcePool resourcePool, ResourcePoolDeleteOptions options)
		{
			...
			Delete(resourcePool.Id, options);
		}
		...
		public void Delete(ResourcePool oToDelete)
		{
			...
			var defaultOptions = new ResourcePoolDeleteOptions();
			Delete(oToDelete.Id, defaultOptions);
		}
		...
		{
			var defaultOptions = new ResourcePoolDeleteOptions();
			Delete(apiObjectId, defaultOptions);
		}
		...
		public void Delete(Guid resourcePoolId, ResourcePoolDeleteOptions options)
		{
			...
			if (!DomResourcePoolHandler.TryDelete(PlanApi, [poolToDelete], out var result, options))
			{
				result.ThrowSingleException(poolToDelete.Id);
			}
		}
		...
		public IReadOnlyCollection<ResourcePool> Complete(IEnumerable<Guid> resourcePoolIds)
		{
			...
			var resourcePools = Read(resourcePoolIds);
			if (!DomResourcePoolHandler.TryComplete(PlanApi, resourcePools?.ToList(), out var result))
			{
				result.ThrowBulkException();
			}
		...
		/// <returns>A read-only collection containing the updated resource pools.</returns>
		/// <exception cref="ArgumentNullException">Thrown when <paramref name="apiObjects"/> is <c>null</c>.</exception>
