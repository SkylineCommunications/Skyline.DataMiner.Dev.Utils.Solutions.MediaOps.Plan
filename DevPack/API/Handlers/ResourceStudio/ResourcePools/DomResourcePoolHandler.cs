	...
	internal class DomResourcePoolHandler : DomInstanceApiObjectValidator<DomResourcePool>
	{
		...
		private void CreateCategoryItems(ICollection<Tuple<DomInstanceId, Guid>> poolInstanceIdsWithCategory)
		{
			...
			foreach (var poolToRegister in poolInstanceIdsWithCategory)
			{
				...
				{
					planApi.Logger.Debug(this, $"Category with ID '{poolToRegister.Item2}' not found for resource pool with instance ID '{poolToRegister.Item1}'. Skipping category item creation for this resource pool.");
					continue;
				}
