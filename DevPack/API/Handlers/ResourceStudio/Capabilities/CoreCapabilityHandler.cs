	...
	internal class CoreCapabilityHandler : ParameterApiObjectValidator
	{
		...
		private void ValidateDiscretes(ICollection<Capability> apiCapabilities)
		{
			foreach (var capability in apiCapabilities)
			{
				...
				var discretesWithInvalidLength = capability.Discretes.Where(x => !InputValidator.HasValidTextLength(x)).ToArray();
				if (discretesWithInvalidLength.Length > 0)
				{
					var error = new CapabilityDiscreteInvalidLengthError
					{
						ErrorMessage = $"{discretesWithInvalidLength.Length} discrete(s) exceed(s) the maximum length of {InputValidator.DefaultMaxTextLength} characters.",
						Id = capability.Id,
